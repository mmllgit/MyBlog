...
var findNumberIn2DArray = function(matrix, target) {
    ...
    let row = 0, column = matrix[0].length - 1 //初始化行和列
    while(row < matrix.length && column >= 0){ //当行列值合法时
        if(matrix[row][column] === target){ //如果相等，返回true
            return true
        }else if(matrix[row][column] > target){ //如果大于则列数减少
            column --
        }else{ //如果小于，行数增加
            row ++
        }
...
var nextGreaterElements = function(nums) {
    ...
    for(let i = 0; i < len * 2; i ++){ //本题最多只循环两次，所以len乘以2
        const j = i % len //循环下标
...
var combinationSum2 = function(candidates, target) {
    ...
    function dfs(sum, cur, index){
        ...
        for(let i = index; i < len; i ++){
            ...
            if (i > index && candidates[i] === candidates[i - 1]) { // 剪枝
                continue
            }
            if(!map.has(i)){ //如果未使用过，进行递归
