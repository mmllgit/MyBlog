...
var isBalanced = function(root) {
    ...
    function dfs(root){
        if(!root || !flag){ //如果为空树或者flag已经为false，则不再处理
            return 0
        }
        const left = dfs(root.left) //计算左子树高度
        const right = dfs(root.right) //计算右子树高度
        if(Math.abs(left - right) > 1){ //比较左右子树高度差是否大于1
            flag = false
            return 0
        }
...
var binaryTreePaths = function(root) {
    ...
    function dfs(root, cur){
        ...
        if(!root.left && !root.right){ //如果左右子树都为空，则将此路径加入res
...
var threeSumClosest = function(nums, target) {
    ...
    for(let i = 0; i < len - 2; i ++){
        ...
        while(j < k){ 
            ...
            if(Math.abs(target - sum) < Math.abs(target - ans)){ //判断本次之差与ans的差的大小
                ans = sum //如果小于则改变abs
            }
            if(sum < target){ //判断本次之和与taget大小，如果小于则左指针向前
                j ++
            }else if(sum > target){ //大于右指针向后
                k --
            }else{ //如果存在等于则直接返回
                return ans
            }
